          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between h-16">
              ...
              <div className="flex items-center space-x-4">
                {user && (
                  <>
                    <Link to="/charts" className="text-[#00f3ff] hover:text-[#00f3ff]/80 flex items-center gap-2">
                      <LineChart className="h-5 w-5" />
                      Charts
                    </Link>
                    <Link to="/news" className="text-[#00f3ff] hover:text-[#00f3ff]/80 flex items-center gap-2">
                      <Newspaper className="h-5 w-5" />
                      News
                    </Link>
                    <Link to="/dashboard" className="text-[#00f3ff] hover:text-[#00f3ff]/80 flex items-center gap-2">
                      <TrendingUp className="h-5 w-5" />
                      Dashboard
                    </Link>
                  </>
                )}
                <Link to="/developers" className="text-[#00f3ff] hover:text-[#00f3ff]/80 flex items-center gap-2">
                ...
                {user ? (
                  <div className="flex items-center space-x-4">
                    ...
                    <button onClick={handleSignOut} className="cyber-button px-4 py-2 rounded-md flex items-center space-x-2">
                      <LogOut className="h-4 w-4" />
                      <span>Disconnect</span>
                    </button>
                  </div>
                ) : (
                  <button onClick={signInWithGoogle} className="cyber-button px-4 py-2 rounded-md flex items-center space-x-2">
              ...
              <div className="cyber-border bg-[#0a0a1f]/80 p-6 rounded-lg">
                ...
                {provider && signer ? (
                  <PaymentForm 
                    ...
                    signer={signer} 
                    onSuccess={(txHash) => {
                      // Redirect back to original site if specified
                      const params = new URLSearchParams(window.location.search);
                      const redirectUrl = params.get('redirect_url');
                      if (redirectUrl) {
                        window.location.href = `${redirectUrl}?tx_hash=${txHash}`;
                      }
                    }}
                  />
                ) : (
                  <div className="text-center">
                    <p className="text-gray-400 mb-6">Please connect your wallet to complete this payment.</p>
                    <button
                      onClick={() => {
                        if (window.ethereum) {
                          if (window.ethereum) {
